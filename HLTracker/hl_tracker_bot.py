...
def cmd_status() -> str:
    ...
    for address, data in wallets.items():
        ...
        lines += f"\n\n👤 <b>{label}</b>\n<code>{address}</code>"
        lines += f"\n🏦 Acct: <b>{fmt_usd(acct_val)}</b>  |  PnL: {pnl_str(total_upnl)}"
        ...
        if positions:
            for asset, pos in positions.items():
                ...
                lines  += (
                    f"\n  {'🔴' if szi < 0 else '🟢'} <b>{asset}</b> {side} {lev_val}x"
                    f"  {fmt_size(abs(szi))} ({fmt_usd(value)})"
                    f"  Entry {fmt_usd(entry)}  Mark {fmt_usd(mark)}"
                    f"  {pnl_str(upnl, pos)}"
                )
        else:
            lines += "\n  (no open positions)"
