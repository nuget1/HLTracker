...
def cmd_add(parts: list):
    if len(parts) < 2:
        tg_send(
            "❌ Usage:\n"
            "/add <code>0xADDRESS Nickname, 0xADDRESS2 Nickname2</code>\n\n"
            "Single:   /add 0xABC Whale Guy\n"
            "Multiple: /add 0xABC Whale Guy, 0xDEF Sniper, 0xGHI Degen"
        )
        return

    # Rejoin everything after /add, then split on ", " for multiple entries
    raw     = " ".join(parts[1:])
    ...
    for entry in entries:
        ...
        label   = " ".join(tokens[1:]).strip() if len(tokens) > 1 else short_addr(address)

        if not address.startswith("0x") or len(address) < 10:
            tg_send(f"❌ Invalid address: <code>{address}</code> — must start with 0x")
            continue
        ...
        # Each wallet added in its own background thread
...
def cmd_del(parts: list) -> str:
    if len(parts) < 2:
        return (
            ...
            "/del <code>0xADDRESS</code>\n\n"
            "Single:   /del 0xABC\n"
            "Multiple: /del 0xABC, 0xDEF, 0xGHI"
        )

    # Rejoin and split on ", " for multiple entries
    raw       = " ".join(parts[1:])
    ...
    for query in queries:
        ...
        with state_lock:
            ...
            if query in wallets:
                ...
                if len(matches) == 1:
                    ...
                    results.append(f"⚠️ <code>{query[:10]}...</code> matched multiple wallets ({labels}) — use full address")
                    continue
                else:
        ...
        save_state()
        results.append(f"🗑️ Stopped tracking <b>{label}</b>  <code>{short_addr(address)}</code>")
...
def msg_fill_group(fills: list, address: str) -> str:
    ...
    return (
        ...
        f"📦 Total Size: <b>{fmt_usd(notional)}</b>\n"
